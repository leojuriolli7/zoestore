 ...
 *
 * It will fetch all blobs from the vercel blob store and all products from the database,
...
async function findUnusedBlobs(blobUrls: string[]): Promise<string[]> {
  ...
  for (let i = 0; i < blobUrls.length; i += BATCH_SIZE) {
    ...
    const existenceChecks = batch.map(async (url: string) => {
      const exists = await db
        .select({ exists: sql<boolean>`1` })
        .from(products)
        .where(sql`${products.image_url} = ${url}`)
        .limit(1)
    ...
    const batchResults = await Promise.all(existenceChecks);
    const batchUnusedUrls = batchResults
